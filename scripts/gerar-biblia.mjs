...
// Map abbrev to safe file name (no leading digit issue)
const FILE_NAME_MAP = {
  jó: 'job',
...
// Map abbrev to valid JS variable name
const VAR_NAME_MAP = {
  jó: 'job',
  '1sm': '_1sm', '2sm': '_2sm', '1rs': '_1rs', '2rs': '_2rs',
  '1cr': '_1cr', '2cr': '_2cr', '1co': '_1co', '2co': '_2co',
...
async function gerarLivro(traducao, abrev, slug, totalCaps) {
  ...
  for (let cap = 1; cap <= totalCaps; cap++) {
    chapterPromises.push(fetchChapter(traducao, slug, cap));
  }
  ...
  if (caps === 0) {
    console.log(`    [${traducao}] ${slug}: 0 capítulos, pulando`);
    return null;
  }
...
async function gerarTraducao(traducao) {
  ...
  for (const [abrev, slug] of Object.entries(LIVRO_MAP)) {
  ...
  const entries = infos.map(i => `  '${i.abrev}': ${i.varName},`).join('\n');
...
async function main() {
  console.log('=== GERADOR DE TEXTOS BÍBLICOS ===\n');
