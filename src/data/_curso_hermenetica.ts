import type { Curso } from './cursos';

export const CURSO_HERMENEUTICA: Curso = {
  id: 'hermeneutica-pratica',
  titulo: 'Hermenêutica Prática',
  descricao: 'Aprenda a interpretar a Bíblia corretamente com métodos comprovados. Evite erros comuns e aprofunde seu entendimento.',
  instrutor: 'Sola Scriptura',
  duracao: '5 semanas',
  nivel: 'intermediario',
  categoria: 'Interpretação Bíblica',
  certificado: true,
  modulos: [
    {
      id: 'mod-principios-fundamentais',
      titulo: 'Princípios Fundamentais',
      descricao: 'Os alicerces para uma interpretação bíblica fiel e consistente',
      icone: '🔍',
      aulas: [
        {
          id: 'aula-herm-1-1',
          titulo: 'O que é hermenêutica?',
          tipo: 'texto',
          duracao: '12 min',
          conteudo: `## O que é hermenêutica?

A hermenêutica é a ciência e a arte da interpretação de textos. Quando aplicada especificamente à Bíblia, torna-se um conjunto de princípios e métodos que nos ajudam a descobrir o significado original dos textos sagrados e a aplicá-los corretamente à nossa vida.

### Origem da palavra

O termo "hermenêutica" vem do grego *hermeneuein*, que significa "interpretar" ou "explicar". Alguns estudiosos conectam a palavra ao deus grego Hermes, o mensageiro dos deuses, que tinha a função de traduzir e comunicar as mensagens divinas aos mortais. Embora essa conexão etimológica seja debatida, a ideia central permanece: hermenêutica é sobre transmitir fielmente o significado de uma mensagem.

### Por que a hermenêutica importa?

Todos nós interpretamos textos diariamente — mensagens de texto, notícias, bulas de remédio. Mas a Bíblia é um texto único: é antigo, escrito em culturas diferentes da nossa, e contém revelação divina. Isso torna a interpretação ao mesmo tempo mais necessária e mais desafiadora.

Sem princípios hermenêuticos, corrermos o risco de:
- Atribuir ao texto significados que ele não tem
- Ignorar o que o autor queria comunicar
- Usar a Bíblia para justificar ideias pessoais
- Perder a riqueza do contexto histórico e cultural

### A hermenêutica bíblica e seus ramos

A hermenêutica bíblica se divide em dois aspectos principais:

1. **Hermenêutica** propriamente dita: os princípios e regras para descobrir o significado do texto. É o estudo de *como* interpretar.

2. **Exegese** (do grego *exēgēsis*): a aplicação prática desses princípios a um texto específico. É o ato de *interpretar* um passagem concreta.

### O método fundamental

Todo estudo hermenêutico segue um caminho lógico:

- **Observação**: O que o texto diz? (leia, releia, note detalhes)
- **Interpretação**: O que o texto significa? (contexto, gramática, história)
- **Aplicação**: O que eu devo fazer com isso? (transformação prática)

Esse三点método — observar, interpretar, aplicar — é a espinha dorsal de todo estudo sério da Bíblia. Ignorar qualquer um desses passos leva a distorções.

### Hermenêutica ≠ opinião pessoal

Um erro muito comum é pensar que "interpretar" significa "dar minha opinião sobre o texto". Não! A hermenêutica séria busca o significado *objetivo* do texto — o que o autor quis comunicar ao seu público original. Nossas preferências pessoais, tradições e experiências são importantes, mas estão subordinadas ao que o texto realmente diz.

> "O primeiro dever do intérprete é esquecer por um momento o que ele 'sabe' e ouvir o que o texto está dizendo." — Grant Osborne

### Para quem é esse estudo?

Hermenêutica não é apenas para pastores e teólogos. Todo cristão que lê a Bíblia é, implicitamente, um intérprete. A diferença está na qualidade da interpretação. Este curso foi escrito para qualquer crente que queira ler a Bíblia com mais clareza, profundidade e fidelidade.`,
          versiculosChave: [
            { ref: '2 Timoteo 2:15', texto: 'Esforça-te por apresentar a Deus um obreiro que não tenha de que se envergonhar, que maneje bem a palavra da verdade.' },
            { ref: 'Atos 8:30-31', texto: 'E Philippe, correndo, ouviu que ele lia o profeta Isaías, e disse: Entendes, pois, o que lês? E ele: Como poderei, se ninguém me dirigir?' },
          ],
        },
        {
          id: 'aula-herm-1-2',
          titulo: 'A regra de interpretação pela Bíblia',
          tipo: 'texto',
          duracao: '14 min',
          conteudo: `## A regra de interpretação pela Bíblia

Um dos princípios mais importantes da hermenêutica reformada é chamado de *scriptura interpreta scriptura* — "a Escritura interpreta a Escritura". Isso significa que a melhor chave para entender um passagem difícil é outra passagem clara da própria Bíblia.

### O princípio em ação

Imagine que você encontra uma passagem obscura em 2 Pedro 3:16, onde Paulo escreve "coisas difíceis de entender". O que isso significa? Em vez de especular, buscamos outras passagens que falam sobre a natureza da Palavra de Deus. Encontramos em 2 Timoteo 3:16 que "toda a Escritura é inspirada por Deus e útil". Essa cruz-referência nos ajuda a equilibrar a ideia: sim, há dificuldades, mas a Escritura como um todo é acessível e útil.

### Por que a Bíblia é sua própria intérprete?

Existem razões teológicas e práticas para isso:

**Razão teológica**: Se a Bíblia é inspirada pelo mesmo Deus, ela não pode se contradizer. Passagens que parecem conflitantes, quando estudadas cuidadosamente, revelam harmonia. O problema nunca está no texto, mas na nossa compreensão.

**Razão prática**: O contexto de uma passagem está frequentemente em outro lugar da Bíblia. Os autores do NT citam o AT constantemente, e entender essas citações é essencial para captar o significado.

### Como aplicar na prática

**Passo 1: Identifique o termo ou tema difícil.** Quando encontrar uma palavra ou ideia confusa, não aceite a primeira interpretação que vier à mente.

**Passo 2: Busque outras ocorrências no mesmo livro.** O autor pode ter explicado o conceito em outro lugar da mesma carta.

**Passo 3: Busque no mesmo autor.** Se Paulo usa um termo estranho em Efésios, talvez ele o explique em Romanos ou Colossenses.

**Passo 4: Busque no Testamento como um todo.** O AT frequentemente esclarece conceitos do NT e vice-versa.

**Passo 5: Deixe o claro interpretar o obscuro.** Nunca construa doutrinas com base em uma passagem isolada e difícil. Sempre confirme com passagens claras e abundantes.

### Exemplo prático: "batismo"

A palavra "batismo" (do grego *baptizō*) aparece em vários contextos. Em Atos 2:38, Pedro diz "arrependei-vos e sede batizados". Em 1 Coríntios 10:1-2, Paulo diz que os israelitas foram "batizados em Moisés na nuvem e no mar" — sem água! A regra *scriptura interpreta scriptura* nos mostra que *baptizō* pode significar imersão, afogamento, ou algo figurativo. O contexto de cada passagem determina o significado.

### Limitações desse princípio

*Scriptura interpreta scriptura* não significa que podemos simplesmente juntar versículos aleatórios para criar um significado. É necessário respeitar:
- O contexto literário de cada passagem
- O contexto histórico-cultural
- O gênero literário (poesia, narrativa, profecia, epístola)
- A intenção do autor inspirado

### A unidade da Bíblia

Quando aplicamos esse princípio corretamente, descobrimos algo maravilhoso: a Bíblia é um livro unitário, com uma mensagem coerente do início ao fim. Cada livro, cada autor, cada seção contribui para o grandioso relato da redenção em Jesus Cristo.`,
          versiculosChave: [
            { ref: '2 Pedro 1:20-21', texto: 'Sobretudo, tendes por certo que nenhuma profecia da Escritura é de interpretação particular.' },
            { ref: 'Isaías 28:10', texto: 'Porque mandamento sobre mandamento, mandamento sobre mandamento, regra sobre regra, regra sobre regra, aqui um pouco, ali um pouco.' },
          ],
        },
        {
          id: 'aula-herm-1-3',
          titulo: 'Contexto histórico e cultural',
          tipo: 'texto',
          duracao: '15 min',
          conteudo: `## Contexto histórico e cultural

A Bíblia foi escrita em momentos históricos específicos, por pessoas que viviam em culturas muito diferentes da nossa. Ignorar esse fato é como assistir ao último episódio de uma série sem ter visto os anteriores — você até entende algo, mas perde a maior parte do significado.

### Por que o contexto importa?

Paulo escreveu sua carta aos Coríntios para resolver problemas reais de uma igreja real: divisões, imoralidade sexual, uso indevido dos dons espirituais, questões sobre comida sacrificada aos ídolos. Se ignorarmos esses problemas, lemos a carta como se fosse apenas um conjunto de instruções genéricas — e perdemos a riqueza da mensagem.

### Como reconstruir o contexto histórico

**1. Estude a cultura do autor e do público**

- Que língua falavam? (hebraico, arameu, grego)
- Quais eram suas tradições religiosas?
- Como era a organização social?
- Quais práticas culturais cercavam o texto?

**2. Entenda o período histórico**

- Escritura no período persa (AT tardio) tem influências diferentes do período romano (NT)
- A diáspora judaica moldou profundamente a teologia do NT
- O Segundo Templo (516 a.C. – 70 d.C.) é o pano de fundo de muitos ensinos de Jesus

**3. Conheça as convenções literárias da época**

- Cartas antigas seguiam um formato específico (saudação, corpo, despedida)
- Parábolas usavam elementos familiares ao ouvinte (vinho, sementes, ovelhas)
- Profecias frequentemente usavam imagens do mundo antigo

### Exemplo: a parábola do semeador

Jesus conta a parábola do semeador (Mateus 13:3-9) para um público galileu. Na Galileia, os campos eram estreitos e o caminho entre eles era compactado pelo trânsito. O semeador jogava sementes em todos os tipos de solo — bom, pedregoso, cheio de espinhos e no caminho. Para nós hoje, isso pode parecer desperdício. Para os ouvintes de Jesus, era a realidade agrícola do dia.

### Ferramentas para estudar o contexto

1. **Comentários bíblicos** — oferecem informações sobre o contexto histórico
2. **Dicionários bíblicos** — explicam termos, costumes e lugares
3. **Livros de introdução ao NT/AT** — fornecem panorama histórico detalhado
4. **A própria Bíblia** — muitas passagens explicam o contexto de outras

### Cuidados com o contexto

O contexto histórico não é um "atalho mágico". Algumas dicas importantes:

- **Não reduza o texto à história.** O contexto histórico ilumina, mas a mensagem transcende a cultura original.
- **Não force paralelos modernos.** Nem toda prática antiga tem equivalente hoje.
- **Respeite a distância cultural.** Temos que fazer esforço para entender o que o texto significava *antes* de pensar no que significa para nós.

> "Um texto sem contexto é um pretexto." — Provérbio hermenêutico

### O Espírito Santo e o contexto

Estudar o contexto não substitui a iluminação do Espírito Santo — complementa. Deus usa o nosso esforço de estudo para nos conduzir à verdade. O Espírito não nos poupa o trabalho de estudar; Ele nos guia através dele.`,
          versiculosChave: [
            { ref: 'Atos 17:11', texto: 'Estes foram mais nobres do que os de Tessalônica, pois receberam a palavra com toda a avidez, examinando diariamente as Escrituras para ver se assim era.' },
            { ref: 'Lucas 24:27', texto: 'E, começando por Moises e todos os profetas, explicou-lhes o que de si mesmo constava em todas as Escrituras.' },
          ],
        },
        {
          id: 'aula-herm-1-4',
          titulo: 'Contexto literário e gramatical',
          tipo: 'texto',
          duracao: '14 min',
          conteudo: `## Contexto literário e gramatical

Enquanto o contexto histórico nos conecta ao mundo do autor e do público original, o contexto literário e gramatical nos ajuda a entender *como* a mensagem foi comunicada. A forma como algo é dito é tão importante quanto o que é dito.

### Contexto literário: o que cerca o texto

O contexto literário se refere ao que vem antes e depois de uma passagem. A regra de ouro é: **um versículo isolado quase sempre é um versículo mal compreendido.**

**Princípio da continuidade**: Uma palavra ou ideia em um versículo recebe seu significado do parágrafo, do capítulo e do livro em que está inserida. Por exemplo, "fé" em Romanos 3:28 tem um significado específico que Paulo desenvolve ao longo de todo o capítulo.

**Princípio do fluxo argumentativo**: Em cartas como Romanos e Hebreus, o autor constrói um raciocínio passo a passo. Cortar no meio de um argumento e destacar um versículo pode distorcer completamente o sentido.

**Princípio da repetição temática**: Autores bíblicos frequentemente repetem uma ideia com diferentes palavras. Notar essas repetições ajuda a identificar os temas centrais de um livro ou seção.

### Gramática: a estrutura da mensagem

A gramática é a ferramenta mais objetiva na hermenêutica. Palavras têm significados específicos, e a forma como se relacionam determina o sentido da frase.

**Significado das palavras**:

Cada palavra tem um campo semântico — um conjunto de significados possíveis. O contexto determina qual significado se aplica. Por exemplo, a palavra grega *agape* pode significar "amor", "caridade" ou "devoção", dependendo do contexto.

**Estrutura das frases**:

- **Sujeito e verbo**: Quem fez o quê? É a pergunta mais básica, mas muitas vezes esquecida.
- **Relações de causa e efeito**: Textos bíblicos frequentemente conectam ideias com "porque", "portanto", "assim", "para que".
- **Condicionais**: "Se... então..." — entender as condições é crucial para aplicar o texto corretamente.
- **Comparativos e superlativos**: "Melhor", "maior", "o mais excelente" — indicam relações de valor.

### Exemplo prático: "fé" em Tiago 2:24

Tiago 2:24 diz: "Vedes, pois, que o homem é justificado por obras e não somente pela fé." Isso contradiz Romanos 3:28, que diz: "Concluímos, pois, que o homem é justificado pela fé sem as obras da lei"?

O contexto literário resolve isso:
- Paulo fala de *justificação diante de Deus* — como o pecador é aceito por Deus (fé sem obras da lei)
- Tiago fala de *justificação diante dos homens* — como a fé se demonstra real (fé que produz obras)
- Cada autor usa "fé" e "obras" com nuances diferentes

### Erros gramaticais comuns

1. **Ignorar o tempo verbal**: "Não pecai" (imperativo) é diferente de "vocês não pecaram" (passado)
2. **Ignorar o número**: "Vocês" (plural) é diferente de "você" (singular)
3. **Ignorar a voz verbal**: "Deus vos ama" (ativo) é diferente de "sede amados" (passivo)
4. **Confundir indicativo com imperativo**: "Cristo morreu por nós" (fato) é diferente de "morrei para o pecado" (comando)

### O poder das conjunções

Paulo usa "mas", "portanto", "assim", "porque" com precisão cirúrgica. Esses pequenos conectivos são frequentemente ignorados, mas são fundamentais para o argumento. Uma dica prática: sublinhe todas as conjunções de Romanos 1-8 e observe como elas estruturam o pensamento de Paulo.

### Lei da primeira menção

Quando um conceito aparece pela primeira vez na Bíblia, ele frequentemente estabelece um padrão que influencia todos os usos seguintes. Por exemplo, a primeira menção de "louvor" (Gênesis 4:26, "então began a invocar o nome do SENHOR") lança luz sobre todo o tema do louvor nas Escrituras.`,
          versiculosChave: [
            { ref: 'Tiago 2:24', texto: 'Vedes, pois, que o homem é justificado por obras e não somente pela fé.' },
            { ref: 'Romanos 3:28', texto: 'Concluímos, pois, que o homem é justificado pela fé sem as obras da lei.' },
          ],
        },
        {
          id: 'aula-herm-1-5',
          titulo: 'O intérprete e seus pressupostos',
          tipo: 'texto',
          duracao: '13 min',
          conteudo: `## O intérprete e seus pressupostos

Ninguém lê a Bíblia em um vácuo. Cada pessoa traz consigo um conjunto de pressupostos, experiências, tradições e expectativas que influenciam como lê o texto. Reconhecer isso não é um problema — é o primeiro passo para uma interpretação honesta.

### O que são pressupostos?

Pressupostos são ideias que aceitamos como verdadeiras antes mesmo de examinarmos um texto. Nós não chegamos à Bíblia como uma "tabela em branco". Trazemos:

- **Pressupostos religiosos**: O que aprendemos na igreja, na catequese, nos sermões
- **Pressupostos culturais**: Valores da nossa sociedade (individualismo, igualdade, liberdade)
- **Pressupostos filosóficos**: Como pensamos sobre verdade, moral, realidade
- **Pressupostos pessoais**: Experiências de vida, feridas, sucessos, frustrações

### Por que isso importa?

Se não reconhecemos nossos pressupostos, corremos o risco de ler o texto *à luz deles* em vez de *à luz do texto*. Por exemplo:

- Alguém que cresceu em uma tradição que enfatiza a prosperidade pode ler 3 João 2 ("quero que você prospere e tenha saúde") como uma promessa de riqueza, quando o contexto é uma saudação epistolar convencional.
- Alguém com feridas de autoridade pode ler passagens sobre submissão apenas com ressentimento, sem captar o ensino mais amplo sobre amor e serviço.

### A humildade hermenêutica

A hermenêutica séria exige humildade. Isso significa:

1. **Aceitar que podemos estar errados.** Nossa interpretação não é infalível. A Bíblia é; nós não.

2. **Buscar a opinião de outros.** Estudar com pessoas de diferentes tradições ajuda a ver cegueiras próprias.

3. **Deixar o texto nos desafiar.** Se toda a nossa leitura da Bíblia confirma exatamente o que já acreditamos, provavelmente não estamos lendo direito.

4. **Priorizar o texto sobre a tradição.** A tradição é útil, mas se contradiz o texto claro, o texto tem prioridade.

### Tradição: aliada ou inimiga?

A tradição não é necessariamente ruim. Ela é o fruto do estudo de gerações de cristãos. Lutero, Calvino, Agostinho e outros nos deixaram uma herança hermenêutica valiosa. Porém, a tradição deve ser avaliada pelo texto, não o contrário.

**Regra prática**: Se a sua tradição diz algo que a Bíblia claramente ensina, siga a Bíblia. Se a sua tradição diz algo que a Bíblia não aborda explicitamente, use a sabedoria cristã. Se a sua tradição contradiz a Bíblia, abandone a tradição.

### O papel do Espírito Santo

A hermenêutica não é apenas um exercício acadêmico. O Espírito Santo é o melhor professor da Palavra. João 16:13 diz que Ele "conduzirá a toda a verdade". Isso não substitui o estudo sério — complementa. O estudo é o meio que Deus usa para nos ensinar.

### Aplicação prática

Antes de estudar qualquer passagem, faça uma oração simples: "Senhor, me livra dos meus pressupostos errados. Ilumina o que estou lendo. Me ensina o que tu queres me dizer." Essa atitude de humildade abre a porta para que o Espírito Santo faça o Seu trabalho em nós.`,
          versiculosChave: [
            { ref: 'Provérbios 18:17', texto: 'Na causa do próprio homem, até o seu parece justo; mas vem o outro e o examina.' },
            { ref: '1 Coríntios 2:14', texto: 'Ora, o homem natural não aceita as coisas do Espírito de Deus, porque lhe são loucura; e não pode entendê-las, porque elas se discernem espiritualmente.' },
          ],
        },
        {
          id: 'aula-herm-1-6',
          titulo: 'Quiz: Princípios Fundamentais',
          tipo: 'quiz',
          duracao: '10 min',
          perguntas: [
            {
              id: 'q-herm-1-1',
              pergunta: 'O que significa o termo grego "hermeneuein"?',
              opcoes: ['Predizer o futuro', 'Interpretar ou explicar', 'Ensinar a multidão', 'Traduzir línguas'],
              respostaCorreta: 1,
              explicacao: 'Hermeneuein significa "interpretar" ou "explicar". A hermenêutica é a ciência da interpretação de textos.',
            },
            {
              id: 'q-herm-1-2',
              pergunta: 'Qual é o método fundamental da hermenêutica bíblica?',
              opcoes: ['Leer, memorizar, recitar', 'Observar, interpretar, aplicar', 'Estudar, debater, discutir', 'Ler, meditar, orar'],
              respostaCorreta: 1,
              explicacao: 'O三点método (observar, interpretar, aplicar) é a espinha dorsal de todo estudo sério da Bíblia. Observar o que o texto diz, interpretar o que significa, e aplicar à vida.',
            },
            {
              id: 'q-herm-1-3',
              pergunta: 'O que significa "scriptura interpreta scriptura"?',
              opcoes: ['A Bíblia é melhor em latim', 'A Bíblia não tem contradições', 'A Escritura interpreta a Escritura', 'Cada pessoa interpreta como quer'],
              respostaCorreta: 2,
              explicacao: 'Scriptura interpreta scriptura significa que a melhor chave para entender uma passagem difícil é buscar outras passagens da Bíblia que esclareçam o mesmo tema.',
            },
            {
              id: 'q-herm-1-4',
              pergunta: 'Por que o contexto histórico é importante para interpretar a Bíblia?',
              opcoes: ['Porque a Bíblia é um livro de história', 'Porque o autor escreveu para um público específico', 'Porque só importa o que aconteceu antigamente', 'Porque o passado é mais importante que o presente'],
              respostaCorreta: 1,
              explicacao: 'O contexto histórico importa porque a Bíblia foi escrita por autores reais para públicos reais em momentos históricos específicos. Entender isso ajuda a captar o significado original.',
            },
            {
              id: 'q-herm-1-5',
              pergunta: 'Qual passagem diz "esforça-te por apresentar a Deus um obreiro que não tenha de que se envergonhar, que maneje bem a palavra da verdade"?',
              opcoes: ['2 Timoteo 2:15', 'Hebreus 4:12', 'Tiago 1:22', 'Romanos 12:1-2'],
              respostaCorreta: 0,
              explicacao: '2 Timoteo 2:15 é o versículo-chave que exorta o estudioso da Bíblia a manejar bem a Palavra com precisão e fidelidade.',
            },
            {
              id: 'q-herm-1-6',
              pergunta: 'O que é "humildade hermenêutica"?',
              opcoes: ['Não estudar a Bíblia demais', 'Aceitar que nossa interpretação pode estar errada', 'Apenas ler o que outros já disseram', 'Nunca discordar da tradição'],
              respostaCorreta: 1,
              explicacao: 'Humildade hermenêutica significa reconhecer que nossa interpretação não é infalível e estar aberto a ser corrigido pelo texto bíblico.',
            },
            {
              id: 'q-herm-1-7',
              pergunta: 'Qual é o erro de isolar versículos do contexto?',
              opcoes: ['Usar concordâncias', 'Tirar um versículo do seu parágrafo e livro', 'Ler várias traduções', 'Estudar o grego'],
              respostaCorreta: 1,
              explicacao: 'Isolar versículos do contexto é um dos erros mais comuns. O significado de um versículo depende do parágrafo, capítulo e livro em que está inserido.',
            },
            {
              id: 'q-herm-1-8',
              pergunta: 'O que são "pressupostos" no estudo hermenêutico?',
              opcoes: ['Versículos que decoramos', 'Ideias que aceitamos antes de examinar o texto', 'Regras gramaticais do grego', 'Opiniões de pastores famosos'],
              respostaCorreta: 1,
              explicacao: 'Pressupostos são ideias que aceitamos como verdadeiras antes de examinarmos o texto. Todos nós temos pressupostos — reconhecê-los é o primeiro passo para uma interpretação honesta.',
            },
            {
              id: 'q-herm-1-9',
              pergunta: 'Qual é a relação entre hermenêutica e exegese?',
              opcoes: ['São a mesma coisa', 'A hermenêutica aplica os princípios; a exegese os estuda', 'A hermenêutica estuda os princípios; a exegese os aplica a um texto', 'Não têm relação entre si'],
              respostaCorreta: 2,
              explicacao: 'A hermenêutica é o estudo dos princípios e regras de interpretação. A exegese é a aplicação prática desses princípios a um texto específico.',
            },
            {
              id: 'q-herm-1-10',
              pergunta: 'Por que "fé" pode ter significados diferentes em Romanos e Tiago?',
              opcoes: ['Porque Paulo e Tiago discordavam', 'Porque cada autor usa a palavra com nuances diferentes em contextos diferentes', 'Porque a Bíblia tem erros', 'Porque as traduções estão erradas'],
              respostaCorreta: 1,
              explicacao: 'Paulo fala de justificação diante de Deus (fé sem obras da lei), enquanto Tiago fala de justificação diante dos homens (fé que produz obras). Cada autor usa "fé" com nuances diferentes.',
            },
          ],
        },
      ],
    },
    {
      id: 'mod-generos-literarios',
      titulo: 'Gêneros Literários',
      descricao: 'Entenda os diferentes tipos de literatura da Bíblia e como interpretar cada um',
      icone: '📚',
      aulas: [
        {
          id: 'aula-herm-2-1',
          titulo: 'Literatura narrativa',
          tipo: 'texto',
          duracao: '14 min',
          conteudo: `## Literatura narrativa

A narrativa é o gênero mais abundante da Bíblia. Mais de 40% do AT e quase metade do NT são narrativas — histórias de pessoas, nações e eventos. Entender como a narrativa bíblica funciona é essencial para não cometer erros de interpretação.

### Características da narrativa bíblica

**1. História orientada a um propósito**

A Bíblia não é um livro de história no sentido moderno. Os autores bíblicos não estavam interessados em cronologia precisa ou detalhes geográficos exatos. Eles escolheram e organizaram os fatos para comunicar verdades teológicas. Isso não significa que a história não é factual — significa que o propósito é teológico, não meramente cronológico.

**2. Caracterização indireta**

Nas narrativas bíblicas, personagens são revelados pelo que fazem, dizem e pensam — não por descrições psicológicas diretas. Para entender um personagem, observe:
- Suas ações (o que ele faz?)
- Suas palavras (o que ele diz?)
- As consequências (o que acontece depois?)
- A avaliação do narrador (se houver)

**3. O narrador bíblico é confiável**

Ao contrário de romances modernos, onde o narrador pode ser não-confiável, o narrador bíblico sempre apresenta a verdade. Quando评价 um personagem ou evento, podemos confiar na sua avaliação.

### Como interpretar narrativas

**Princípio 1: Não faça o que o personagem faz sem confirmar com outros textos.** Davi cometeu adultério e assassinato — mas a narrativa não endossa essas ações. Abraão mentiu sobre sua esposa — mas isso não é uma instrução para nós.

**Princípio 2: Observe o contexto literário maior.** Uma narrativa no livro de Juizes frequentemente termina com a frase "cada um fazia o que lhe parecia reto". Isso é avaliação, não aprovação.

**Princípio 3: Busque o propósito teológico.** Pergunte: por que o autor incluiu essa história? O que ela ensina sobre Deus, sobre o povo, sobre o plano redentor?

### Principais narrativas do AT

- **Gênesis 1-11**: Narrativa primordial (criação, queda, dilúvio, Babel)
- **Gênesis 12-50**: Patriarcas (Abraão, Isaque, Jacó, José)
- **Êxodo 1-15**: Libertação do Egito
- **Livro de Rute**: Lealdade e providência no período dos juizes
- **1-2 Samuel**: Monarquia de Saul e Davi
- **Livro de Jonas**: Misericórdia de Deus e os limites da nação de Israel

### Narrativas do NT

- **Evangelhos**: Biografias teológicas de Jesus — cada um com ênfase diferente
- **Atos dos Apostolos**: A expansão da igreja doJerusalém a Roma

### Erros comuns com narrativas

1. **Moralismo**: Tirar uma "lição moral" de cada história sem considerar o propósito teológico
2. **Alegorização**: Ver significados ocultos em cada detalhe
3. **Modelo automático**: Assumir que o personagem deve ser imitado
4. **Isolamento**: Ler a história fora do livro em que está inserida

> A narrativa bíblica não é um manual de instruções — é uma história que nos convida a participar da narrativa de Deus.`,
          versiculosChave: [
            { ref: 'Romanos 15:4', texto: 'Porque tudo o que foi escrito, para nosso ensino foi escrito, para que, pela paciência e pela consolação das Escrituras, tenhamos esperança.' },
            { ref: '1 Coríntios 10:11', texto: 'Tudo isso lhes aconteceu como exemplo, e foi escrito para advertência de nós, sobre quem chegaram os fins dos séculos.' },
          ],
        },
        {
          id: 'aula-herm-2-2',
          titulo: 'Literatura poética',
          tipo: 'texto',
          duracao: '15 min',
          conteudo: `## Literatura poética

A poesia ocupa um lugar central na Bíblia. O Salter é o livro mais citado no NT, e Jesus mesmo citou Salmos frequentemente. Mas a poesia bíblica tem regras próprias que precisamos entender para não interpretar errado.

### Características da poesia hebraica

**1. Paralelismo**

A poesia hebraica não rima — ela usa paralelismo. Duas ou mais linhas se relacionam de maneiras específicas:

- **Sinônimo**: A segunda linha repete a ideia da primeira com palavras diferentes
  > "O SENHOR é o meu pastor; nada me faltará." (Salmo 23:1)

- **Antitético**: A segunda linha contrasta com a primeira
  > "O SENHOR conhece o caminho dos justos, mas o caminho dos ímpios perecerá." (Salmo 1:6)

- **Sintético**: A segunda linha expande ou complementa a primeira
  > "Clama a mim, e responder-te-ei, e anunciar-te-ei coisas grandes e firmes, que não sabes." (Jeremias 33:3)

**2. Metáfora e linguagem figurada**

A poesia bíblica é repleta de imagens:
- Deus é "rocha", "escudo", "luz", "pai"
- O pecador é "ovelha perdida", "inimigo"
- A salvação é "libertação", "cura", "perdão"

Tomar essas figuras literalmente é um erro grave. Quando o Salmo diz "Deus é a minha fortaleza", ninguém pensa em uma fortaleza física.

**3. Hipérbole (exagero)**

> "Meus ossos se quebram, porque eu clamo o dia inteiro" (Salmo 22:17)

Davi não estava com ossos literalmente partidos — ele expressava a intensidade da sua dor.

### Livros poéticos principais

- **Salmos**: Louvores, orações, lamentações — a hymnbook de Israel
- **Provérbios**: Máximas de sabedoria prática
- **Eclesiastes**: Reflexão filosófica sobre o sentido da vida
- **Cântico dos Cantos**: Poesia de amor entre noivo e noiva
- **Lamentações**: Poesia de luto pela destruição de Jerusalém

### Como interpretar poesia

1. **Não tire doutrinas literais de linguagem figurada.** "Os montes saltam como cordeiros" (Salmo 114:4) não é um evento geológico.

2. **Observe o paralelismo.** Ele revela o significado que o poeta quis comunicar.

3. **Respeite a intenção emocional.** A poesia expressa sentimentos profundos — não é um tratado teológico.

4. **Considere o contexto do salmo.** Um salmo de lamentação não deve ser lido como uma promesa de prosperidade.

### Cuidados com a poesia

A poesia bíblica é frequentemente mal usada:
- Salmos de lamentação são transformados em promessas
- Metáforas são interpretadas literalmente
- Hipérboles são tratadas como fatos

A chave é sempre perguntar: "O autor estava sendo literal ou figurado? Como eu sei isso?"`,
          versiculosChave: [
            { ref: 'Salmo 19:1', texto: 'Os céus narram a glória de Deus, e o firmamento anuncia a obra das suas mãos.' },
            { ref: 'Provérbios 3:5-6', texto: 'Confia no SENHOR de todo o teu coração, e não te estribes no teu próprio entendimento. Reconhece-o em todos os teus caminhos, e ele endireitará as tuas veredas.' },
          ],
        },
        {
          id: 'aula-herm-2-3',
          titulo: 'Literatura profética',
          tipo: 'texto',
          duracao: '14 min',
          conteudo: `## Literatura profética

A literatura profética é uma das mais mal compreendidas da Bíblia. Muita gente abre o livro de Apocalipse ou Isaías e se perde em símbolos, visões e linguagem apocalíptica. Entender os gêneros proféticos é a chave para destrancar esses livros.

### Três tipos de literatura profética

**1. Profecia preditiva (futura)**

O profeta anuncia eventos que ainda não aconteceram no momento da fala. Exemplo:
> "Viria um menino nasceria de uma virgem" (Isaías 7:14 — cumprimento em Mateus 1:23)

**2. Profecia de julgamento (condicional)**

O profeta anuncia consequências que podem ser evitadas通过 arrependimento. Exemplo:
> "Se o povo se arrepender, Deus não trará o julgamento" (Jeremias 18:7-10)

**3. Profecia de restauração (consolação)**

O profeta anuncia a restauração de Deus para o povo depois do julgamento. Exemplo:
> "Eu restaurarei os cativos do meu povo" (Jeremias 30:3)

### A literatura apocalíptica

Apocalipse (e partes de Daniel e Ezequiel) pertence ao gênero apocalíptico, que tem características próprias:

- **Simbolismo intenso**: Números, cores, animais e objetos representam ideias
- **Visões e sonhos**: O autor vê imagens que precisam ser interpretadas
- **Linguagem cifrada**: Frequentemente usa imagens do AT para comunicar verdades

### Como interpretar profecia

1. **Identifique o gênero**: É preditiva, de julgamento ou de restauração?
2. **Observe o público original**: Para quem o profeta falou? O que isso significava para eles?
3. **Respeite o simbolismo**: Nem tudo é literal no apocalipse
4. **Busque o cumprimento**: A profecia se cumpriu? Em parte? Ainda vai se cumprir?
5. **Cuidado com timelines**: Muitos estudiosos erram ao tentar montar cronogramas detalhados

### Erros comuns com profecia

- **Focalizar o futuro em vez de Jesus**: O objetivo da profecia não é satisfazer curiosidade sobre o futuro, mas revelar o caráter e os planos de Deus
- **Forçar cumprimentos contemporâneos**: Nem toda profecia se cumprou ainda
- **Ignorar o contexto original**: A profecia tinha significado para os ouvintes primeiros
- **Confundir simbolismo com literalidade**: "Sete trombetas" pode não significar sete trombetas físicas

### O princípio cristológico

Toda profecia aponta para Cristo. Isaías 53 fala do Servo Sofredor. Daniel 7 fala do Filho do Homem. Apocalipse 19 fala do Cavaleiro fiel. Quando você não sabe como interpretar uma profecia, pergunte: "Onde está Jesus aqui?"`,
          versiculosChave: [
            { ref: '2 Pedro 1:20-21', texto: 'Sobretudo, tendes por certo que nenhuma profecia da Escritura é de interpretação particular.' },
            { ref: 'Apocalipse 1:3', texto: 'Bem-aventurado aquele que lê e os que ouvem as palavras desta profecia, e guardam o que nela está escrito; porque o tempo está próximo.' },
          ],
        },
        {
          id: 'aula-herm-2-4',
          titulo: 'Literatura epistolar',
          tipo: 'texto',
          duracao: '13 min',
          conteudo: `## Literatura epistolar

O Novo Testamento contém 21 cartas — mais da metade dos seus livros. Entender como funcionavam as cartas antigas é fundamental para interpretar Paulo, Tiago, Pedro, João e o autor de Hebreus.

### A carta antiga vs. a carta moderna

| Aspecto | Carta Antiga | Carta Moderna |
|---------|-------------|---------------|
| Público | Comunidade inteira | Indivíduo |
| Leitura | Pública (lida em voz alta) | Privada |
| Propósito | Resolver problemas reais | Expressar sentimentos |
| Formato | Saudação → Corpo → Despedida | Variado |

### Estrutura típica de uma carta paulina

1. **Saudação** (nome do autor, destinatários, saudação de graça)
2. **Agradecimento/oração** (agradece a Deus pelo destinatário)
3. **Corpo da carta** (argumentação, ensino, exortação)
4. **Instruções práticas** (aplicações concretas)
5. **Despedida** (saudações finais, bênção)

Essa estrutura não é formalidade — é conteúdo. A saudação em Romanos 1:7 já contém verdades teológicas sobre a graça e a paz.

### Como interpretar cartas

**1. Entenda o problema que o autor está respondendo**

As cartas são respostas a situações específicas. Paulo não escreveu Romanos como um tratado teológico abstrato — ele escreveu para uma igreja que ele nunca visitou, apresentando o evangelho que pregava.

**2. Não separe versículos do argumento**

Efésios 2:8-9 ("por graça sois salvos...") é uma frase de Paulo dentro de um argumento que começou no capítulo 1. Tirar do contexto é como pular no meio de uma conversa.

**3. Diferencie o que é cultural do que é universal**

Quando Paulo diz "as mulheres devem calar-se na igreja" (1 Coríntios 14:34), precisamos entender: isso era universal ou resolvia um problema específico daquela comunidade? A mesma pergunta vale para "não useis joias" (1 Timoteo 2:9).

**4. Respeite a retórica do autor**

Paulo usa ironia (Gálatas 5:12), sarcasmo (2 Coríntios 12:13), e humor. Ignorar isso leva a interpretações distorcidas.

### Principais cartas

- **Romanos**: A exposição mais completa da doutrina da justificação
- **1 Coríntios**: Respostas a problemas práticos em uma igrejaProblemática
- **Efésios**: A igreja como corpo de Cristo
- **Filipenses**: Alegria em meio à adversidade
- **Hebreus**: A superioridade de Cristo sobre o sistema levítico

### A universalidade da mensagem

Apesar de endereçadas a situações específicas, as cartas contêm verdades universais. O desafio é discernir: o que era contextual e o que é permanente? A resposta geralmente está em outros textos da mesma Bíblia.`,
          versiculosChave: [
            { ref: '2 Timoteo 3:16', texto: 'Toda a Escritura é inspirada por Deus e útil para o ensino, para a repreensão, para a correção, para a educação na justica.' },
            { ref: 'Romanos 15:4', texto: 'Porque tudo o que foi escrito, para nosso ensino foi escrito.' },
          ],
        },
        {
          id: 'aula-herm-2-5',
          titulo: 'Literatura sapiencial',
          tipo: 'texto',
          duracao: '12 min',
          conteudo: `## Literatura sapiencial

A literatura sapiencial compreende livros que lidam com as questões fundamentais da vida: sofrimento, justiça, prudência, prazer e o temor a Deus. Provérbios, Jó, Eclesiastes e Cantico dos Canticos são os principais representantes dessa tradição.

### Características da sabedoria

**1. Sabedoria prática**

Provérbios oferece máximas para a vida cotidiana. São verdades gerais, não promessas absolutas:
> "Instrui o menino no caminho em que deve andar, e até quando envelhecer não se desviará dele." (Provérbios 22:6)

Isso é uma verdade geral — não uma lei infalível. Existem pais fiéis cujos filhos se desviaram, e pais negligentes cujos filhos se arrependeram.

**2. Sabedoria experimental**

Jó e Eclesiastes questionam as simplificações de Provérbios. Jó sofre apesar de ser justo. Eclesiastes diz que "tudo é vaidade". Eles não contradizem Provérbios — expandem a sabedoria para além das fórmulas fáceis.

**3. Sabedoria relacional**

Cântico dos Canticos celebra o amor humano como reflexo do amor divino. A tradição cristã sempre viu nele uma imagem do amor entre Cristo e a igreja — sem negar o sentido literal.

### Como interpretar cada livro

**Provérbios**:
- Entenda que são provérbios, não promessas
- Cada provérbio é uma verdade geral que precisa de sabedoria para aplicar
- Nem toda promessa se cumpre literalmente em cada situação
- O contexto do livro (o temor do Senhor é o princípio da sabedoria) é essencial

**Jó**:
- Não é um livro sobre "por que os justos sofrem" — é sobre os limites do conhecimento humano
- Os amigos de Jó falam muita coisa errada — o livro não endossa as suas teologias
- A resposta de Deus no final não explica o sofrimento — revela Sua soberania

**Eclesiastes**:
- O "vaidade" (hebraico *hebel*) significa "vapor", "efêmero"
- O autor não é cético — é realista sobre os limites da experiência humana
- A conclusão é "teme a Deus e guarda os seus mandamentos" (12:13)

**Cântico dos Canticos**:
- Não force alegoria em cada verso — o sentido literal é válido
- Considere tanto a interpretação literal quanto a tipológica

### Erros comuns

1. **Tratar Provérbios como promessas absolutas**
2. **Ignorar o tom de Jó e Eclesiastes**
3. **Alegorizar Cântico dos Canticos demais**
4. **Confundir sabedoria com conhecimento** — sabedoria é saber aplicar o conhecimento`,
          versiculosChave: [
            { ref: 'Provérbios 1:7', texto: 'O temor do SENHOR é o princípio do conhecimento; os insensatos desprezam a sabedoria e a instrução.' },
            { ref: 'Eclesiastes 12:13', texto: 'Teme a Deus e guarda os seus mandamentos; porque isso é o dever de todo homem.' },
            { ref: 'Jó 42:2-3', texto: 'Eu sei que tudo podes, e que nenhum dos teus propósitos pode ser frustrado. Quem é este que sem saber fala? Diria eu o que não entendo, coisas maravilhosas demais para mim.' },
          ],
        },
        {
          id: 'aula-herm-2-6',
          titulo: 'Quiz: Gêneros Literários',
          tipo: 'quiz',
          duracao: '10 min',
          perguntas: [
            {
              id: 'q-herm-2-1',
              pergunta: 'Qual é a característica principal da poesia hebraica?',
              opcoes: ['Rima no final dos versos', 'Paralelismo entre linhas', 'Métrica regular', 'Estrofes organizadas'],
              respostaCorreta: 1,
              explicacao: 'A poesia hebraica usa paralelismo em vez de rima. As linhas se relacionam por sinonímia, antítese ou síntese, comunicando ideias de maneira paralela.',
            },
            {
              id: 'q-herm-2-2',
              pergunta: 'Qual tipo de profecia pode ser evitada com arrependimento?',
              opcoes: ['Profecia preditiva absoluta', 'Profecia de julgamento condicional', 'Profecia de restauração', 'Profecia apocalíptica'],
              respostaCorreta: 1,
              explicacao: 'A profecia de julgamento condicional é acompanhada de uma condição: se o povo se arrepender, Deus não trará o julgamento (Jeremias 18:7-10).',
            },
            {
              id: 'q-herm-2-3',
              pergunta: 'Qual livro da Bíblia é classificado como "hineário de Israel"?',
              opcoes: ['Provérbios', 'Salmos', 'Lamentações', 'Cântico dos Canticos'],
              respostaCorreta: 1,
              explicacao: 'O Salter (Salmos) é o hineário de Israel — uma coleção de cânticos, orações e louvores usados no culto do templo e que permanecem centrais na adoração judaica e cristã.',
            },
            {
              id: 'q-herm-2-4',
              pergunta: 'Quando você encontra uma narrativa na Bíblia, o que deve perguntar primeiro?',
              opcoes: ['Isso é literal?', 'Qual é a moral da história?', 'Por que o autor incluiu essa história?', 'Como posso aplicar isso hoje?'],
              respostaCorreta: 2,
              explicacao: 'O primeiro passo é entender o propósito teológico do autor. Por que ele incluiu essa história no livro? O que ela ensina sobre Deus e o plano redentor?',
            },
            {
              id: 'q-herm-2-5',
              pergunta: 'Qual é o erro mais comum ao interpretar Provérbios?',
              opcoes: ['Ler em português', 'Tratar provérbios como promessas absolutas', 'Comparar com outros livros', 'Estudar o contexto histórico'],
              respostaCorreta: 1,
              explicacao: 'Provérbios são verdades gerais, não promessas absolutas. "Instrui o menino no caminho" é uma máxima de sabedoria, não uma garantia infalível.',
            },
            {
              id: 'q-herm-2-6',
              pergunta: 'Qual é a conclusão de Eclesiastes?',
              opcoes: ['Tudo é inútil, não faça nada', 'Aproveite a vida enquanto pode', 'Teme a Deus e guarda os seus mandamentos', 'A sabedoria não tem valor'],
              respostaCorreta: 2,
              explicacao: 'Eclesiastes 12:13 é a conclusão: "Teme a Deus e guarda os seus mandamentos; porque isso é o dever de todo homem." Apesar de toda a "vaidade", Deus é o centro.',
            },
            {
              id: 'q-herm-2-7',
              pergunta: 'O que diferencia as cartas do NT de cartas modernas?',
              opcoes: ['Eram escritas em grego', 'Eram lidas publicamente em comunidades inteiras', 'Não tinham saudação', 'Eram sempre anônimas'],
              respostaCorreta: 1,
              explicacao: 'As cartas do NT eram lidas em voz alta em reuniões da igreja. Eram dirigidas a comunidades, não apenas a indivíduos, e continham ensino e exortação para toda a congregação.',
            },
            {
              id: 'q-herm-2-8',
              pergunta: 'O que significa "hebel" em Eclesiastes?',
              opcoes: ['Perfeito', 'Vapor ou efêmero', 'Eterno', 'Secreto'],
              respostaCorreta: 1,
              explicacao: 'Hebel (הֶבֶל) significa "vapor" ou "efêmero". Eclesiastes usa essa palavra repetidamente para descrever a natureza passageira das coisas humanas.',
            },
            {
              id: 'q-herm-2-9',
              pergunta: 'Qual é o princípio cristológico na interpretação de profecia?',
              opcoes: ['Toda profecia é sobre o Antigo Testamento', 'Toda profecia aponta para Cristo', 'Profecia é apenas sobre o futuro', 'Profecia não tem relação com Jesus'],
              respostaCorreta: 1,
              explicacao: 'Toda profecia bíblica aponta, direta ou indiretamente, para Cristo. Isaías 53 fala do Servo Sofredor. Daniel 7 do Filho do Homem. Apocalipse 19 do Cavaleiro fiel.',
            },
            {
              id: 'q-herm-2-10',
              pergunta: 'Qual é a diferença entre sabedoria e conhecimento na literatura sapiencial?',
              opcoes: ['Não há diferença', 'Conhecimento é informação; sabedoria é saber aplicar', 'Sabedoria é para velhos; conhecimento para jovens', 'Conhecimento é bíblico; sabedoria não é'],
              respostaCorreta: 1,
              explicacao: 'Conhecimento é acumular informações. Sabedoria é saber como aplicar essas informações na vida real, com discernimento e temor a Deus.',
            },
          ],
        },
      ],
    },
    {
      id: 'mod-erros-comuns',
      titulo: 'Erros Comuns de Interpretação',
      descricao: 'Identifique e evite os equívocos mais frequentes na leitura da Bíblia',
      icone: '⚠️',
      aulas: [
        {
          id: 'aula-herm-3-1',
          titulo: 'Alegorização excessiva',
          tipo: 'texto',
          duracao: '13 min',
          conteudo: `## Alegorização excessiva

A alegorização é um dos erros mais sedutores da interpretação bíblica. Ela consiste em atribuir significados ocultos ou simbólicos a detalhes que o autor provavelmente pretendia que fossem literais. Embora a Bíblia use simbolismo (especialmente em profecia e poesia), a alegorização excessiva distorce o texto.

### Origem histórica

A alegorização foi popularizada por Clemente de Alexandria e Orígenes no século III, e dominou a interpretação cristã por séculos. Santo Agostinho, por exemplo, interpretou a parábola do bom samaritano de forma alegórica:
- O homem que desceu de Jerusalém para Jericó = Adão
- Os ladrões = Adão e a mulher
- O sacerdote = Lei
- O levita = Profecias
- O bom samaritano = Cristo
- A hospedaria = Igreja

Essa interpretação é criativa, mas não tem base no texto. Jesus explicou explicitamente que o bom samaritano era "aquele que praticou a misericórdia" (Lucas 10:36) — não uma alegoria de Cristo.

### Quando a alegoria é válida?

A alegoria *pode* ser legítima quando:
- O próprio autor bíblico a usa (Gálatas 4:21-31 — Paulo alegoriza Sara e Hagar)
- O contexto indica claramente que é simbólico (Apocalipse, parábolas)
- A interpretação é consistente com o resto da Bíblia

### sinais de alegorização excessiva

1. **Cada detalhe precisa ter um significado oculto.** Se a parábola do semeador tivesse significado para cada tipo de solo, cada pássaro e cada pedra, seria impossível entender.

2. **O significado "alegórico" é subjetivo.** Duas pessoas podem encontrar significados completamente diferentes no mesmo texto, sem critério objetivo.

3. **O texto original não suporta a interpretação.** Se você precisa de um dicionário de símbolos para entender o que o autor disse, provavelmente está impondo algo externo ao texto.

4. **O significado literal é ignorado.** A alegorização muitas vezes substitui o sentido claro do texto por algo mais "profundo" — mas esse "profundo" é inventado.

### O caso das águas em Apocalipse

Em Apocalipse, "águas" frequentemente representam povos ou nações (Ap 17:15). Mas em outros contextos, águas são literalmente águas (batismo em Romanos 6:3-4). A chave é: o contexto determina se uma imagem é simbólica ou literal.

### O perigo da alegorização

1. **Tira a objetividade do texto.** Se tudo é alegoria, nada é literal — e o texto perde sua capacidade de nos julgar.

2. **Coloca o intérprete acima do autor.** Em vez de descobrir o que o autor quis dizer, o intérprete inventa o que o texto "realmente" significa.

3. **Leva a interpretações contraditórias.** Sem critério objetivo, cada pessoa acha que sua alegoria é a correta.

### O caminho correto

Respeite o gênero literário. Não alegorize narrativas históricas. Não force símbolos onde o texto é claro. E sempre pergunte: "O que o autor provavelmente quis dizer com isso para o seu público original?"`,
          versiculosChave: [
            { ref: 'Lucas 10:36', texto: 'Qual, pois, destes três te parece que foi o próximo daquele que caiu nas mãos dos salteadores?' },
            { ref: '2 Pedro 1:20', texto: 'Sobretudo, tendes por certo que nenhuma profecia da Escritura é de interpretação particular.' },
          ],
        },
        {
          id: 'aula-herm-3-2',
          titulo: 'Isolar versículos do contexto',
          tipo: 'texto',
          duracao: '14 min',
          conteudo: `## Isolar versículos do contexto

O versículo isolado é o maior inimigo da interpretação bíblica correta. Frases tiradas do contexto se tornam armas nas mãos de quem quer provar qualquer coisa. A hermenêutica séria exige que leiamos versículos dentro do seu parágrafo, capítulo e livro.

### Por que isso acontece?

Vivemos na era do "versículo do dia" e do "frase de efeito". Redes sociais e calendários devocionais favorecem versículos curtos e impactantes. Isso não é necessariamente ruim — mas quando não voltamos ao contexto, corremos o risco de distorção.

### Exemplos clássicos de versículos fora de contexto

**1. "Deus os amou de tal maneira que deu o seu Filho unigênito" (João 3:16)**

Esse versículo é verdadeiro, mas isolado de João 3:14-15, que fala da serpente de bronze no deserto (Números 21:4-9). Jesus está dizendo: "Assim como a serpente foi levantada, assim o Filho do Homem será levantado" — uma referência ao juízo sobre o pecado, não apenas ao amor abstrato.

**2. "Posso todas as coisas naquele que me fortalece" (Filipenses 4:13)**

Esse versículo é frequentemente usado como motivação para empreendimentos pessoais. Mas o contexto (Filipenses 4:10-19) fala de contentamento em toda situação — tanto na abundância quanto na necessidade. Paulo está dizendo que pode enfrentar qualquer circunstância, não que pode realizar qualquer sonho.

**3. "Pois onde está o teu tesouro, aí estará também o teu coração" (Mateus 6:21)**

Isolado, parece uma frase motivacional sobre investimentos. No contexto (Mateus 6:19-24), Jesus está falando sobre os perigos do dinheiro e da ganância. O tesouro que importa é o celestial, não o terreno.

**4. "Não julgueis, para que não sejais julgados" (Mateus 7:1)**

Muitos usam isso para proibir qualquer forma de julgamento. Mas Mateus 7:5 diz "tira primeiro a trave do teu olho" — o que implica que, depois de se corrigir, pode ajudar o irmão. E João 7:24 manda "julgai com justo juízo". O contexto é sobre julgamento hipócrita, não sobre discernimento.

### O princípio hermenêutico

**Regra de ouro**: Nenhum versículo pode significar algo que contradiga o restante da Bíblia. Se a sua interpretação de um versículo isolado contradiz claramente outros textos, a interpretação está errada — não a Bíblia.

### Como voltar ao contexto

1. Leia o parágrafo inteiro
2. Leia o capítulo
3. Identifique o argumento do livro
4. Consulte outros textos sobre o mesmo tema
5. Pergunte: "O que isso significava para o público original?"

### A prática devocional e o contexto

Estudo devocional e estudo hermenêutico não são mutuamente exclusivos. Você pode meditar em um versículo todos os dias — mas sempre voltando ao contexto para não perder o significado real. O versículo é a porta de entrada; o contexto é a casa inteira.`,
          versiculosChave: [
            { ref: 'Mateus 7:1', texto: 'Não julgueis, para que não sejais julgados.' },
            { ref: 'Mateus 7:5', texto: 'Hipócrita, tira primeiro a trave do teu olho, e então verás claramente para tirar aLasca do olho do teu irmão.' },
            { ref: 'Filipenses 4:13', texto: 'Posso todas as coisas naquele que me fortalece.' },
          ],
        },
        {
          id: 'aula-herm-3-3',
          titulo: 'Projetar a cultura moderna no texto antigo',
          tipo: 'texto',
          duracao: '13 min',
          conteudo: `## Projetar a cultura moderna no texto antigo

Um dos erros mais sutis e perigosos na hermenêutica é ler a Bíblia com os óculos da cultura moderna. Quando fazemos isso, o texto diz o que nós queremos ouvir em vez do que o autor quis comunicar.

### O que é projeção cultural?

Projeção cultural é assumir que as nossas categorias, valores e experiências são automaticamente aplicáveis ao mundo bíblico. Por exemplo:
- Assumir que "família" no NT significa a mesma coisa que "família" no Brasil do século XXI
- Assumir que "igreja" no NT significa um prédio com púlpito e bancos
- Assumir que "pobre" no AT significa a mesma coisa que pobreza no nosso contexto

### Exemplos de projeção cultural

**1. O conceito de "família"**

No mundo bíblico, a família era extensa — pais, filhos, avós, tios, primos, servos, todos vivendo na mesma propriedade. Quando Jesus diz "quem ama pai ou mãe mais do que a mim não é digno de mim" (Mateus 10:37), ele está falando contra um laço familiar que para nós é impensável romper.

**2. O conceito de "pobreza"**

Os "pobres" no AT (anawim) não eram apenas financeiramente necessitados — eram os humildes, os oprimidos, os sem poder político. A pobreza bíblica tem uma dimensão social e espiritual que nossa categoria moderna ignora.

**3. O conceito de "escravidão"**

A escravidão no mundo bíblico era diferente da escravidão atlântica. Não era baseada em raça, e muitos escravos eram tratados como membros da família. Isso não significa que a escravidão era justa — mas entender a diferença é crucial para interpretar corretamente os textos que a mencionam.

**4. O conceito de "culto"**

Quando dizemos "vamos ao culto", imaginamos um prédio, um púlpito, hinos e um sermão. O culto no AT era sacrifício animal no templo. O culto no NT era reunião em casas, com ceia do Senhor, louvor e ensino. As nossas categorias modernas de culto são diferentes das bíblicas.

### Como evitar a projeção cultural

1. **Estude a cultura original.** Antes de aplicar um texto, entenda como as pessoas viviam, pensavam e organizavam suas sociedades.

2. **Respeite a distância cultural.** Reconheça que há 2.000 ou 3.000 anos, as coisas eram diferentes. Não force equivalências.

3. **Diferencie o princípio do padrão.** O princípio de amor e justiça é universal; o padrão cultural pode ser contextual.

4. **Não envergonhe a Bíblia.** Não tente fazer a Bíblia parecer "moderna". Ela é antiga — e é exatamente isso que a torna relevante. Ela fala de um mundo diferente com verdades universais.

> "O texto bíblico não foi escrito para nós, mas foi preservado para nós." — Kevin DeYoung`,
          versiculosChave: [
            { ref: '1 Coríntios 9:20-22', texto: 'Tornei-me judeu para os judeus, para ganhar os judeus; para os que estão debaixo da lei, como se estivesse debaixo da lei...'} ,
            { ref: 'Romanos 12:2', texto: 'E não vos conformeis com este mundo, mas transformai-vos pela renovação do vosso entendimento.' },
          ],
        },
        {
          id: 'aula-herm-3-4',
          titulo: 'Confundir descrição com prescrição',
          tipo: 'texto',
          duracao: '12 min',
          conteudo: `## Confundir descrição com prescrição

Uma das confusões mais comuns na interpretação bíblica é assumir que, porque algo é descrito na Bíblia, é também prescrito (ordenado). A Bíblia muitas vezes descreve o que aconteceu sem endossar o que aconteceu.

### A diferença crucial

- **Descrição**: O que aconteceu. Um fato narrado.
- **Prescrição**: O que deve acontecer. Uma instrução ou mandamento.

A Bíblia descreve muitas coisas que não são prescritas:
- Davi cometeu adultério (2 Samuel 11) — a Bíblia descreve, mas não prescreve
- Abraão mentiu sobre sua esposa (Gênesis 12) — a Bíblia descreve, mas não prescreve
- Judas se enforcou (Mateus 27:5) — a Bíblia descreve, mas não prescreve

### Onde isso causa problemas

**1. A poligamia no AT**

Jacó teve duas esposas e duas concubinas. A Bíblia descreve isso sem condenação explícita. Mas isso significa que poligamia é aceitável? O texto descreve, mas não prescreve. O padrão estabelecido em Gênesis 2:24 é "um homem e uma mulher".

**2. A violência no AT**

Deus ordena a destruição completa de Amaleque (1 Samuel 15:3). A Bíblia descreve essa ordem. Mas isso significa que devemos destruir nossos inimigos hoje? A ordem era contextual — para um povo específico, em um momento específico, com um propósito teológico específico.

**3. A escravidão no NT**

Paulo envia Onesimo de volta a Filemom como "irmão amado" (Filemom 16). A Bíblia descreve essa situação. Mas isso significa que escravidão é aceitável? O NT descreve a escravidão como realidade social, mas o princípio subjacente é dignidade humana e amor ao próximo.

**4. Os votos no AT**

Ana fez um voto a Deus e depois o cumpriu (1 Samuel 1). A Bíblia descreve isso. Mas Jesus ensina: "não jureis nem pelo céu nem pela terra" (Mateus 5:34-37). A descrição do AT é superada pela prescrição do NT.

### Como distinguishir descrição de prescrição

1. **Busque a avaliação do autor.** O autor bíblico aprova, condena ou é neutro em relação ao que descreve?

2. **Considere o contexto mais amplo.** O que outros textos da Bíblia ensinam sobre o mesmo tema?

3. **Observe se há comando.** O texto está dizendo "faça isso" ou está dizendo "isso aconteceu"?

4. **Diferencie o testemunho do mandamento.** A Bíblia testemunha sobre muitas coisas que não manda.

### O perigo da confusão

Quando confundimos descrição com prescrição:
- Usamos a Bíblia para justificar práticas que ela apenas descreve
- Ignoramos princípios claros porque encontramos exceções narrativas
- Criamos contradições internas na Bíblia que não existem

### O critério final

O critério para distinguir descrição de prescrição é: **O que Jesus ensinou e praticou?** Ele é o ápice da revelação de Deus. Tudo o que precedeEle deve ser lido à luz dEle.`,
          versiculosChave: [
            { ref: 'Mateus 5:17-18', texto: 'Não cuideis que vim destruir a Lei ou os Profetas; não vim para destruir, mas para cumprir.' },
            { ref: 'Hebreus 1:1-2', texto: 'Havendo Deus antigamente falado de muitos modos aos nossos pais, por meio dos profetas, nestes últimos tempos nos falou pelo Filho.' },
          ],
        },
        {
          id: 'aula-herm-3-5',
          titulo: 'Aplicação correta do Antigo Testamento',
          tipo: 'texto',
          duracao: '15 min',
          conteudo: `## Aplicação correta do Antigo Testamento

O Antigo Testamento é frequentemente negligenciado ou mal interpretado. Muitos cristãos ignoram o AT porque "é do tempo da lei", ou o lêem de forma literal demais, aplicando rituais e leis que não se aplicam mais. Entender como o NT se relaciona com o AT é essencial para uma hermenêutica saudável.

### A relação entre AT e NT

**1. Continuidade e descontinuidade**

O NT não abroga o AT — ele o cumpre. Jesus disse: "Não vim para destruir a Lei, mas para cumprir" (Mateus 5:17). Mas cumprir não significa que todas as leis do AT continuam em vigor. Existem categorias:

- **Leis morais** (10 Mandamentos, princípios de amor e justiça): Continuam válidas porque refletem o caráter eterno de Deus
- **Leis cerimoniais** (sacrifícios, festas, pureza ritual): Foram cumpridas em Cristo e não se aplicam mais
- **Leis civis** (organização social de Israel): Tinham validade para nação específica, não para a igreja

**2. Tipologia**

Muitos elementos do AT são "tipos" — figuras que apontam para Cristo:
- O cordeiro pascal → Cristo, o Cordeiro de Deus (João 1:29)
- O maná no deserto → Cristo, o pão da vida (João 6:32-35)
- A serpente de bronze → Cristo levantado na cruz (João 3:14)
- O tabernáculo → Cristo como lugar da presença de Deus (João 1:14)

**3. O AT como sombra**

Hebreus 10:1 diz que "a Lei tem uma sombra dos bens futuros, não a imagem exata das coisas". O AT é como uma silhueta — aponta para algo maior, mas não é o objeto em si.

### Como aplicar o AT corretamente

1. **Leia à luz de Cristo.** Todo o AT encontra seu cumprimento em Jesus. Gênesis aponta para a semente da mulher. Os Profetas apontam para o Messias. Os Salmos expressam a oração que Cristo cumpriu.

2. **Identifique a categoria da lei.** É moral, cerimonial ou civil? Isso determina se a lei continua ou não em vigor.

3. **Respeite o contexto histórico.** As leis de Israel eram para Israel. Não podemos simplesmente transplantar leis agrílicas do AT para a sociedade moderna.

4. **Busque o princípio teológico.** Mesmo as leis cerimoniais ensinam verdades permanentes sobre pecado, santidade e expiação.

5. **Não negligencie o AT.** Paulo diz que "toda a Escritura é inspirada por Deus" (2 Timoteo 3:16) — e quando ele escreveu isso, a "Escritura" era o AT.

### Erros comuns

- **Antinomianismo**: "A lei não vale mais, então posso fazer o que quiser" — o NT confirma os princípios morais do AT
- **Legalismo**: "Preciso obedecer todas as leis do AT para agradar a Deus" — Cristo cumpriu a lei por nós
- **Literalismo cego**: Aplicar leis cerimoniais ou civis de Israel à igreja hoje
- **Negligência**: Ignorar o AT como se não tivesse relevância

### A beleza do AT

O AT não é um "livro velho e obsoleto". Ele é a história da fidelidade de Deus mesmo quando o povo falhava. É a preparação para o evangelho. É a raiz na qual o NT cresce. Sem o AT, perdemos metade da história de Deus com a humanidade.`,
          versiculosChave: [
            { ref: 'Mateus 5:17', texto: 'Não cuideis que vim destruir a Lei ou os Profetas; não vim para destruir, mas para cumprir.' },
            { ref: 'Hebreus 10:1', texto: 'Porque, tendo a Lei uma sombra dos bens futuros, não a imagem exata das coisas, nunca pode, pelos mesmos sacrifícios que se offerecem continuamente cada anno, aperfeiçoar os que a eles se chegam.' },
            { ref: '2 Timoteo 3:16', texto: 'Toda a Escritura é inspirada por Deus e útil para o ensino.' },
          ],
        },
        {
          id: 'aula-herm-3-6',
          titulo: 'Quiz: Erros Comuns',
          tipo: 'quiz',
          duracao: '10 min',
          perguntas: [
            {
              id: 'q-herm-3-1',
              pergunta: 'O que é alegorização excessiva?',
              opcoes: ['Ler a Bíblia em alegria', 'Atribuir significados ocultos a detalhes que o autor pretendeu literais', 'Estudar a Bíblia com alegria', 'Interpretar parábolas'],
              respostaCorreta: 1,
              explicacao: 'A alegorização excessiva consiste em atribuir significados simbólicos a detalhes que o autor provavelmente pretendia que fossem literais, como detalhes geográficos ou numéricos.',
            },
            {
              id: 'q-herm-3-2',
              pergunta: 'Qual é o problema de usar Filipenses 4:13 ("Posso todas as coisas") fora de contexto?',
              opcoes: ['O versículo é falso', 'Foi escrito para pessoas específicas', 'O contexto fala de contentamento em toda situação, não de realizar sonhos pessoais', 'O versículo só se aplica a Paulo'],
              respostaCorreta: 2,
              explicacao: 'No contexto de Filipenses 4:10-19, Paulo fala de contentamento tanto na abundância quanto na necessidade. Ele pode enfrentar qualquer circunstância — não realizar qualquer sonho pessoal.',
            },
            {
              id: 'q-herm-3-3',
              pergunta: 'Qual é a diferença entre descrição e prescrição na Bíblia?',
              opcoes: ['Descrição é do AT, prescrição é do NT', 'Descrição é o que aconteceu; prescrição é o que deve acontecer', 'Não há diferença', 'Descrição é mais importante'],
              respostaCorreta: 1,
              explicacao: 'A Bíblia descreve muitas coisas que aconteceram (adultério de Davi, poligamia de Jacó) sem necessariamente prescrever (ordenar) essas práticas. Distinguir o que é narrado do que é ordenado é crucial.',
            },
            {
              id: 'q-herm-3-4',
              pergunta: 'Por que não devemos projetar a cultura moderna no texto bíblico?',
              opcoes: ['Porque a Bíblia é perfeita', 'Porque o autor escreveu para um público específico com categorias diferentes', 'Porque a cultura moderna é superior', 'Porque a Bíblia não tem nada a ver com a vida real'],
              respostaCorreta: 1,
              explicacao: 'O autor bíblico escreveu para pessoas com categorias, valores e experiências diferentes dos nossos. Projetar a cultura moderna faz o texto dizer o que nós queremos ouvir em vez do que o autor quis comunicar.',
            },
            {
              id: 'q-herm-3-5',
              pergunta: 'Como o NT se relaciona com as leis cerimoniais do AT?',
              opcoes: ['Elas continuam válidas', 'Cristo as cumpriu, então não se aplicam mais', 'Nunca foram importantes', 'Devemos segui-las literalmente'],
              respostaCorreta: 1,
              explicacao: 'As leis cerimoniais (sacrifícios, festas, pureza ritual) foram cumpridas em Cristo. Ele é o sacrifício perfeito, o verdadeiro cordeiro pascal. Portanto, essas leis não se aplicam mais à igreja.',
            },
            {
              id: 'q-herm-3-6',
              pergunta: 'Qual exemplo de alegorização é considerado inadequado?',
              opcoes: ['Paulo alegorizando Sara e Hagar em Gálatas 4', 'Santo Agostinho alegorizando a parábola do bom samaritano', 'Jesus usando parábolas', 'João escrevendo Apocalipse'],
              respostaCorreta: 1,
              explicacao: 'Agostinho interpretou detalhes da parábola como alegorias (Adão, Cristo, a igreja), mas Jesus explicou que o bom samaritano era "aquele que praticou a misericórdia" — uma interpretação moral, não alegórica.',
            },
            {
              id: 'q-herm-3-7',
              pergunta: 'O que significava "pobre" (anawim) no contexto bíblico?',
              opcoes: ['Apenas sem dinheiro', 'Os humildes, oprimidos e sem poder político', 'Pessoas preguiçosas', 'Estrangeiros'],
              respostaCorreta: 1,
              explicacao: 'Os anawim no AT não eram apenas financeiramente necessitados — eram os humildes, os oprimidos socialmente e os sem poder político. A pobreza bíblica tem uma dimensão mais ampla que a nossa categoria moderna.',
            },
            {
              id: 'q-herm-3-8',
              pergunta: 'Por que "não julgueis" (Mateus 7:1) não proíbe todo julgamento?',
              opcoes: ['Porque Jesus não sabia do que falava', 'Porque Mateus 7:5 e João 7:24 ensinam discernimento', 'Porque o versículo é opcional', 'Porque Paulo discorda de Jesus'],
              respostaCorreta: 1,
              explicacao: 'Mateus 7:5 diz "tira primeiro a trave do teu olho" (implicando que depois pode ajudar), e João 7:24 manda "julgai com justo juízo". O versículo proíbe julgamento hipócrita, não discernimento.',
            },
            {
              id: 'q-herm-3-9',
              pergunta: 'Qual é o critério final para distinguir descrição de prescrição?',
              opcoes: ['O que o pastor diz', 'O que Jesus ensinou e praticou', 'O que a maioria dos cristãos faz', 'O que é mais confortável'],
              respostaCorreta: 1,
              explicacao: 'Jesus é o ápice da revelação de Deus. Tudo o que precedeEle deve ser lido à luz dEle. O que Ele ensinou e praticou é o critério definitivo para entender o que é prescrito.',
            },
            {
              id: 'q-herm-3-10',
              pergunta: 'Qual é o erro do antinomianismo no que diz respeito ao AT?',
              opcoes: ['Leer demais o AT', 'A Lei não vale mais, então posso fazer o que quiser', 'Seguir todas as leis do AT', 'Ignorar o Novo Testamento'],
              respostaCorreta: 1,
              explicacao: 'O antinomianismo é o erro de pensar que, porque a lei cerimonial foi cumprida, todos os mandamentos morais também foram abolidos. O NT confirma os princípios morais do AT.',
            },
          ],
        },
      ],
    },
  ],
};
