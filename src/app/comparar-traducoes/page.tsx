...
import { ArrowRight, ChevronDown, Eye, EyeOff, Layers, BookOpen, Loader2, AlertCircle, Search, X } from "lucide-react";
import ScrollReveal from "@/components/ScrollReveal";
import { cn } from "@/lib/utils";
import { TODOS_LIVROS, LIVROS_AT, LIVROS_NT, type LivroInfo } from "@/data/biblia/livros";
import { carregarTraducao, obterCapituloMulti, type CapituloComparado, type VersiculoData } from "@/data/biblia/texto/carregar";

const TRADUCOES_COMPARACAO = [
  { id: "arc", nome: "ARC", cor: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20" },
  { id: "ara", nome: "ARA", cor: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
  { id: "acf", nome: "ACF", cor: "text-purple-600 dark:text-purple-400", bg: "bg-purple-50 dark:bg-purple-900/20" },
  { id: "nvi", nome: "NVI", cor: "text-rose-600 dark:text-rose-400", bg: "bg-rose-50 dark:bg-rose-900/20" },
  { id: "kjv", nome: "KJV", cor: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-900/20" },
  { id: "web", nome: "WEB", cor: "text-cyan-600 dark:text-cyan-400", bg: "bg-cyan-50 dark:bg-cyan-900/20" },
] as const;
...
function compararTextos(base: string, alvo: string): { diffs: [number, number][] } {
  const wordsBase = base.split(/\s+/);
  const wordsAlvo = alvo.split(/\s+/);
  const diffs: [number, number][] = [];
  const maxLen = Math.max(wordsBase.length, wordsAlvo.length);
  for (let i = 0; i < maxLen; i++) {
    const wb = wordsBase[i]?.toLowerCase().replace(/[.,;:!?'"()]/g, "");
    const wa = wordsAlvo[i]?.toLowerCase().replace(/[.,;:!?'"()]/g, "");
    if (wb !== wa) {
      diffs.push([i, i]);
    }
  }
...
const TRAD_LABELS: Record<string, string> = {
  arc: "ARC", ara: "ARA", acf: "ACF", nvi: "NVI", kjv: "KJV", web: "WEB",
};

export default function CompararTraducoesPage() {
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const resultado = await obterCapituloMulti(livro.abreviacao, capitulo, tradsAtivas);
  ...
  const versiculosFiltrados = useMemo(() => {
    ...
    const maxVersos = Math.max(...dados.map(d => d.versiculos.length));
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      dados.forEach(d => {
        const v = d.versiculos.find(x => x.numero === num);
        if (v) traducoes[d.traducao] = v.texto;
      });
      versiculos.push({ numero: num, traducoes });
    }
    return versiculos;
  }, [dados, versiculoInicio, versiculoFim]);

  const tradsArray = useMemo(() => Array.from(tradsVisiveis).filter(id => TRADUCOES_COMPARACAO.some(t => t.id === id)), [tradsVisiveis]);
      ...
      <main className="pt-20 pb-16 px-4 sm:px-6">
        <div className="max-w-[1400px] mx-auto">
          <ScrollReveal>
            <div className="text-center mb-8">
              <div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-[#d4a853]/20 to-amber-500/20 flex items-center justify-center border border-[#d4a853]/20">
                <ArrowRight className="w-8 h-8 sm:w-10 sm:h-10 text-[#d4a853]" />
              </div>
          ...
          <AnimatePresence mode="wait">
            {carregando && (
              ...
              >
                ...
                <p className="text-sm text-[var(--content-muted)]">
                  {t("compare.loading", "Carregando traduções...")}
                </p>
              </motion.div>
            )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={cn(
                          ...
                          tradsArray.length >= 6 && "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3",
                        )}>
                          {tradsArray.map((tradId, tIdx) => {
                            const texto = verso.traducoes[tradId];
                            if (!texto) return null;
                            const tradInfo = TRADUCOES_COMPARACAO.find(x => x.id === tradId);
            ...
            {!carregando && !erro && versiculosFiltrados.length === 0 && !carregando && (
              <motion.div
                key="empty"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-[var(--surface-sunken)]/20 flex items-center justify-center">
                  <ArrowRight className="w-7 h-7 text-[var(--content-muted)]/40" />
                </div>
    ...
    <>
      {words.map((item, i) => (
        <span
          key={i}
          className={cn(
            item.isDiff && "bg-amber-200/60 dark:bg-amber-500/20 rounded px-0.5 text-amber-800 dark:text-amber-200"
          )}
        >
          {item.word}{" "}
        </span>
      ...
      <p className="text-[10px] leading-relaxed text-[var(--content-muted)]/70 font-mono italic space-x-3">
        {palavras.map((p, i) => {
          ...
          const gregos: Record<string, { palavra: string; def: string }> = {
            ...
            "filhos de deus": { palavra: "τέκνα θεοῦ", def: "Filhos de Deus" },
            "palavra de deus": { palavra: "λόγος θεοῦ", def: "Palavra de Deus" },
          };
          const traducaoHebraica: Record<string, { palavra: string; def: string }> = {
          ...
          const dict = testamento === "NT" ? gregos : traducaoHebraica;
