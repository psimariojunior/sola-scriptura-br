...
import { Brain, Trophy, Clock, ChevronRight, RotateCcw, Check, X } from 'lucide-react';
          ...
          <div className="max-w-2xl mx-auto">
            <ScrollReveal>
              <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                  <h1 className="font-display text-3xl font-light">Quiz Bíblico</h1>
                  <p className="text-sm text-muted-foreground">Teste seus conhecimentos das Escrituras</p>
                </div>
              </div>
              <div className="space-y-6">
                ...
                {ranking.length > 0 && (
                  <div className="rounded-2xl border border-border/50 bg-card/50 p-6">
                    <div className="flex items-center gap-2 mb-4"><Trophy className="w-4 h-4 text-amber-500" /><h3 className="text-sm font-semibold">Ranking</h3></div>
                      ...
                      {ranking.slice(0, 10).map((item, i) => (
                        <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-background/50">
                          <div className="flex items-center gap-3">
                            ...
                            <div><p className="text-sm font-medium">{item.nome}</p><p className="text-xs text-muted-foreground">{NIVEIS_QUIZ[item.nivel]?.label}</p></div>
                          </div>
          ...
          <div className="max-w-2xl mx-auto">
            <ScrollReveal>
              ...
              <div className="flex gap-3">
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setTela('inicio')} className="flex-1 py-3 border border-border rounded-xl font-medium hover:bg-muted/50 transition-all">
                  Menu
                </motion.button>
              </div>
            </ScrollReveal>
