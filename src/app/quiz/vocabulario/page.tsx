...
import {
  Brain, RotateCcw, Check, X, BookOpen, ArrowRight,
  Sparkles, Timer, Trophy, Star, Zap, ChevronLeft,
  Layers, Target, Volume2, Hash, Flame, Swords,
  Award, AlignLeft, Type, Languages,
  type LucideIcon,
} from "lucide-react";
...
const DIFICULDADE_LABELS: Record<Dificuldade, { label: string; desc: string; icon: LucideIcon }> = {
  facil: { label: "Fácil", desc: "Palavras comuns (freq. ≥ 50)", icon: Star },
  medio: { label: "Médio", desc: "Todas as palavras", icon: Flame },
  dificil: { label: "Difícil", desc: "Palavras raras (freq. ≤ 5)", icon: Swords },
};

const TIPO_PERGUNTA_ICONS: Record<Question["tipo"], LucideIcon> = {
  definicao: AlignLeft,
  palavra: Type,
  strong: Hash,
  livro: BookOpen,
...
export default function QuizVocabularioPage() {
  ...
  if (carregando) {
    return (
      <PageShell maxWidth="5xl" className="flex items-center justify-center">
  ...
  return (
    <PageShell maxWidth="5xl" noContainer>
        {/* Confetti effect */}
        {showConfetti && (
              ...
              {modo === "quiz" && !quizAtivo && (
                <div className="flex items-center gap-1 bg-card rounded-xl border border-border p-1">
                  {(Object.keys(DIFICULDADE_LABELS) as Dificuldade[]).map((d) => {
                    const DifIcon = DIFICULDADE_LABELS[d].icon;
                    return (
                      <button
                        key={d}
                      ...
                      >
                        <DifIcon className="w-3.5 h-3.5" /> {DIFICULDADE_LABELS[d].label}
                      </button>
                    );
                  })}
                </div>
                        ...
                        <div className={cn("flashcard-inner w-full min-h-[320px] relative", flipped && "flipped")}>
                          ...
                          <div className="flashcard-front absolute inset-0 bg-card rounded-2xl border-2 border-primary/30 golden-glow p-8 flex flex-col items-center justify-center">
                            ...
                            <span className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                              <Languages className="w-3.5 h-3.5" /> {currentFlashcard.word.idioma === "grego" ? "Grego" : "Hebraico"}
                            </span>
            ...
            {modo === "quiz" && !quizAtivo && !quizFinalizado && (
              <div className="max-w-xl mx-auto text-center">
                <div className="bg-card rounded-2xl border border-border p-8">
                  ...
                  <h2 className="text-h2 mb-2">Quiz Bíblico</h2>
                  <p className="text-muted-foreground mb-6">
                      ...
                      return (
                        ...
                        >
                          <span className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                            <DifIcon className="w-5 h-5 text-primary" />
                          </span>
                          <div>
              ...
              <div className="max-w-xl mx-auto">
                ...
                <div className="flex justify-center mb-4">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium border border-primary/20">
                    {(() => {
                      const TipoIcon = TIPO_PERGUNTA_ICONS[quizPerguntas[quizIdx].tipo];
                      return <TipoIcon className="w-3.5 h-3.5" />;
                    })()}
              ...
              <div className="max-w-xl mx-auto text-center">
                <div className="bg-card rounded-2xl border border-border p-8">
                  ...
                  <div className="flex justify-center gap-3 mb-6">
                    <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-background border border-border text-sm font-medium">
                      {(() => { const DifIcon = DIFICULDADE_LABELS[dificuldade].icon; return <DifIcon className="w-4 h-4" />; })()}
                      {DIFICULDADE_LABELS[dificuldade].label}
                    </span>
                  </div>
